...
from sklearn.model_selection import model_selection.train_test_split
...
def main(args):
    ...
    best = min((np.sqrt(mean_squared_error(Ridge(alpha=x).fit(data_train, target_train).predict(data_test), target_test)), x) for x in lambdas)
    best_lambda = best[1]
    best_rmse = best[0]
